...
export const addDetail = async (newDetail: Omit<IProduct_Detail, 'id_product_detail'| 'product_id'>, productId: number): Promise<number | Error> => {
  try {
    ...
    const [result] = await Knex(EtableNames.product_details).insert({...newDetail, product_id: productId}).returning('id_product_details')

    if(result) return Number(result.id_product_details)

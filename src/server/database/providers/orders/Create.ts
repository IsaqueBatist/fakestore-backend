...
export const create = async (userId: number): Promise<number | Error> => {
  try {
    ...
    const addOrderItems = await Knex(EtableNames.order_items).insert(
      orederItems,
    );

    if (!addOrderItems)
      return new Error("Error while add items from cart to order");
    ...
    //Limpar carrinho
    await Knex(EtableNames.cart_items).where("cart_id", userCart.id_cart).del();
